...
const TechMeetingComponent = {
  ...
  next() {
    ...
    if (this.itemIndex >= TechMeetingService.data[this.currentTopic].length) {
      this.itemIndex = 0;
      this.topicIndex++;
      if(this.getTopic())Speech.speak('let\'s move on to ' + this.getTopic() + ' topics');
    }
  ...
  say(topic, item) {
    Speech.speak(topic + 'topic point ' + (this.itemIndex + 1) + '. ' + item.TITLE + '. ' + item.SPEAKER + ' can you tell us about this?');
  },
  getTopic() {
